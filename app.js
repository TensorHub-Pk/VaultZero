...
async function checkForUpdates() {
    if (window._isCheckingUpdate || State.airGap) return;
 ...
 */
// SECURITY: The only key allowed to sign official updates.
// Ed25519 Public Key (Verified by sign-updates.js)
...
async function initVersionControl() {
    ...
    // If the user clicked "Repair & Refresh", we need to re-verify
    // the freshly downloaded files. If they still fail, it's a
    // Security Alert, not a Repair alert.
    // ---------------------------------------------------------------
    ...
    // Physically check if critical files on the server match the signed manifest
    let deepIntegrityOk = true;
    const criticalToScan = [
        'app.js', 
        'service-worker.js', 
        'encryption.js',
        'image-hide.js',
    ...
    try {
        const fetcher = window._nativeFetch || fetch;
        for (const file of criticalToScan) {
            const res = await fetcher(`${file}?t=` + Date.now());
            if (res.ok) {
                const text = await res.text();
                const actualHash = await sha256Text(text);
                const expectedHash = manifest.hashes[file]?.sha256;

                if (expectedHash && actualHash !== expectedHash) {
                    console.error(`Deep Scan: HASH MISMATCH detected in ${file} on server!`);
                    deepIntegrityOk = false;

                    // Log to Audit Log immediately
                    AuditLog.log(AuditLog.EventType.INTEGRITY_FAIL, {
                        asset: file,
                        msg: `Deep Scan Mismatch (Server-side Tamper Attempt)`
                    });
                }
            }
        }
    ...
    if (serverVersion) {
        ...
        if (hashesChanged || !deepIntegrityOk) {
            ...
            if (!integrityOk || !deepIntegrityOk) {
                ...
                // Hashes changed BUT the manifest signature is INVALID.
                // This is a potential tamper attempt — no repair offered.
                console.error("Update Check: INTEGRITY BREACH - Manifest signature invalid or file mismatch.");
                ...
                const shouldLock = await customConfirm(
                    "A file mismatch was detected but the update manifest has an invalid or missing signature. This could be a tampering attempt.\n\nWould you like to enter Isolation Mode for maximum protection?",
                    "🚨 Security Alert",
                    "Isolation Mode",
                    "Continue Anyway"
                );
                ...
                if (shouldLock) {
                    ...
                    toast("Security warning bypassed. Proceed with caution.", "info");
                }

            } else if (window._isPostRepairVerification) {
                // === POST-REPAIR SUCCESS ===
                ...
                toast("Repair successful. Vault is running the latest verified version.", "success");
    ...
    /**
     * Shows the Repair Modal when a valid, cryptographically-signed
     * update is detected but the local cache is stale.
     * The Repair button will hard-reload to fetch fresh files.
     * On the next load, if files still fail verification → Security Alert fires.
     */
    function showRepairPrompt() {
        ...
        // Wire up the Repair & Refresh button
        document.getElementById('btn-repair-refresh').onclick = async () => {
            // Flag this as a post-repair load so the next verification
            // knows to treat failure as a Security Alert, not show repair again.
            sessionStorage.setItem('vz_repair_pending', '1');

            // Unregister SW to force a clean re-install of the new version
        ...
        document.getElementById('btn-repair-dismiss').onclick = () => {
            modal.classList.remove('active');
            setTimeout(() => modal.classList.add('hidden'), 300);
            toast('Repair dismissed. Your cached version remains active.', 'warning');
        };
    }
    ...
    if ('serviceWorker' in navigator) {
        ...
        navigator.serviceWorker.addEventListener('message', (event) => {
            ...
            if (event.data.type === 'SECURITY_ANOMALY') {
                ...
                if (isCritical) toast(`CRITICAL Security Mismatch: ${asset.split('/').pop()}`, "error");
            }
        });

        // Automatic reload when a new service worker takes over
